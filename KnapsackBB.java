// Knapsack problem solved in Branch and Bound
// author: Ali El Adi
public class KnapsackBB {
	...
	public static double UpperBound(int[] w, int[] p,int n, int ccourante, int j) 
	{
		...
		if (i>j) {
			for(int k=j; k<=i-1 ; k++) u=u+p[k-1]-w[k-1]*p[i-1]/w[i-1];
				}
		return u;
	}
...
int n=7;
int c=50;
int[] p={70,20,39,37,7,5,10,0};
int[] w={31,10,20,19,4,3,6,1000};
...
//System.out.println("j>n"+(j>n));
	...
	if (zcourante>z)
		{z=zcourante;
		 for(int k=1; k<=n;k++) x[k-1]=xcourante[k-1];
		}
	j=n;
	if (xcourante[n-1]==1)
		{ ccourante+=w[n-1];
...
// System.out.println("upperbound"+(j==n || (j<n && z>=zcourante +UpperBound(w,p,n,ccourante,j) )));
	if (j==n || (j<n && z>=zcourante +UpperBound(w,p,n,ccourante,j) ))
			{// 5 backtrack
do {
	i=-1;
	int k=1;
	...
	if (i==-1){
		...
		for(int l=1; l<=n;l++) System.out.print("x["+l+"]="+x[l-1]+" ");
	}
